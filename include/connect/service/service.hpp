...
namespace service {
    /**
     * This Service header is used to implement a ROS2 Service Server and Client as plugin for the ROS2 "connect" node.
     */
    class Service {
    public:
         ...
         */
        virtual void init(Service_Action_t &definition, const rclcpp::Node::SharedPtr & /*node*/, const rclcpp::CallbackGroup::SharedPtr &callbackGroup, const std::string &/*remoteEndpoint*/, const std::function<void(const std::shared_ptr<const std::vector<uint8_t>> &)> send) {
            ...
            this->toRemoveThread = std::thread(&Service::executeRemove, this);
        }
         ...
         * @param nodeServicesInterface the nodes service interface
         * @param callbackGroup the callback group to register the service in
         */
    ...
    protected:
         ...
         * This will asynchronous delete the handle and also joining its thread.
         *
         * @param handle the handle to remove
         */
        virtual void removeHandle(const std::shared_ptr<ServiceActionHandle> &handle) {
